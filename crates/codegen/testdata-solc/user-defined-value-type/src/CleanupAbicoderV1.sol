// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;
pragma abicoder v1;

type MyUInt8 is uint8;

// Note that this wraps from a uint256
function wrap(uint x) pure returns (MyUInt8 y) { assembly { y := x } }
function unwrap(MyUInt8 x) pure returns (uint8 y) { assembly { y := x } }

contract CleanupAbicoderV1 {
    uint8 a;
    MyUInt8 b;
    uint8 c;

    function ret() external pure returns (MyUInt8) {
        return wrap(0x1ff);
    }
    function f(MyUInt8 x) external pure returns (MyUInt8) {
        return x;
    }
    function mem() external pure returns (MyUInt8[] memory) {
        MyUInt8[] memory x = new MyUInt8[](2);
        x[0] = wrap(0x1ff);
        x[1] = wrap(0xff);
        require(unwrap(x[0]) == unwrap(x[1]));
        assembly {
            mstore(add(x, 0x20), 0x1ff)
        }
        require(unwrap(x[0]) == unwrap(x[1]));
        return x;
    }
    function stor() external returns (uint8, MyUInt8, uint8) {
        a = 1;
        c = 2;
        b = wrap(0x1ff);
        return (a, b, c);
    }
}
