...
function readDataSlotZero() view returns (bytes32) {
...
contract CopyByteArrayToStorage {
    uint padding;
    bytes data;

    function f() public returns (uint) {
        bytes32 zero;
        if (!(readDataSlotZero() == zero)) return 1;
        data = "abc";
        if (!(readDataSlotZero() == zero)) return 2;
        data = "1234567890123456789012345678901234567890123456789012345678901234567890";
        if (!(readDataSlotZero() != zero)) return 3;
        ...
        data = "abc";
        if (!(readDataSlotZero() == zero)) return 8;
        ...
        if (!(readDataSlotZero() != zero)) return 12;
