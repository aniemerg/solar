...
contract AbiEncodeCallDeclaration is Base {
    function run() public view returns (uint r) {
        bool success;
        bytes memory result;
        ...
        (success, result) = address(this).staticcall(abi.encodeCall(AbiEncodeCallDeclaration.b, 10000));
        require(success && result.length == 32);
        r += abi.decode(result, (uint));
        require(r == 11116);
